...
| T4.2 | finalized assistant 块级 Markdown 与嵌套列表呈现 | `docs/superpowers/specs/2026-07-20-tui-block-markdown-renderer-design.md` | 🟡 2026-07-20 设计已确认、待计划与实现：把 whole-message fallback 收窄为 block-local fallback，支持最多三层列表与普通闭合 fenced code，保持 streaming/user/draft literal 和 `action-draft` 逐字边界；只修改 TUI presentation，不改变 API、watch、driver、SafetyGate、审批或冻结状态。 |
