...
| T1 | Ink 终端 UI 只读三面板（状态/chat/actions） | T 独立线 | ✅ 2026-07-07 完成：`tui/` 独立 Node/TS/Ink 包，状态/chat/actions 三面板，SSE 优先 + polling/degraded fallback，纯 HTTP API/SSE 客户端 |
| T2/T3 | Ink 交互（审批依赖 F1.3）与补齐 | T 独立线 | 🟡 2026-07-07 T2-lite 完成：chat、`/task`、`/approve`、`/reject`、`/reset true`、`/refresh`、`/help`、`/quit`；T3 config/upload 未做 |
...
| 基建 | CI（pytest 3.11/3.12 + 前端 build）；push 纪律；测试 env-scrub fixture | 流程欠账 | ✅ 2026-07-07 完成轻量 CI：Python 3.11/3.12 pytest、frontend build/e2e、tui build/test；CI env 清代理变量 |
