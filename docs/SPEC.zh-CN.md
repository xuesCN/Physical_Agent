...
| 编号 | 内容 | 归属 | 状态 |
| --- | --- | --- | --- |
| **R0-R8** | 架构减法与兼容面退役 | `specs/001-architecture-simplification/` | 🟡 R0-R2.1 已完成。R3-A `e7f878b` 锁定 sidecar behavior；R3-B `a64be59` 完成 adapter cutover；R3-C `d1713a0` 固化真实历史救援；R3-D `6309a1b` 删除当前 migrator；R3-E `4a8ec86` 删除 full Workspace helper、专用 parser/renderer/public export 与退役 roundtrip tests，driver fixtures 改为专用目录，保留 `WorkspaceDocument` 和 sidecar 最小 Markdown 工具。删除后真实历史救援仍全绿，production/tests 旧协议引用为零，全量 Python `404 passed`。下一阶段 R3-F 当前文档与最终验证收口 |
...
| VNext-0 | Proposal application layer + typed metadata + execution boundary + execution_mode | §2/VNext | ✅ 2026-07-10 本轮完成：主要 task/action 入口复用 `ProposalService` 与统一 planner factory；metadata 增加 v1 typed view/correlation 且不能伪造审批；Chat auto_step 成为兼容 no-op，driver coding 改静态校验；robot 显式区分 simulation/hardware，默认 hardware，hardware 按默认 SAFETY 策略要求审批。**尚不包含统一 Agent loop** |
