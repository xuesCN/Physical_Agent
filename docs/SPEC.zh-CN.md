...
| **B6（独立项：退役 markdown 后端）** | 状态层收口 | 删 `MarkdownStateStore`/factory 分支/config legacy 自动探测/矩阵测试 md 侧与 `test_e2e_markdown_loop`；**保留** markdown renderer/parser（SAFETY 真源、LOG 镜像、audit export 依赖）；`migrate-md-to-sqlite` 留一个版本周期。**必须排在 F1.3 之前**——否则审批元数据要给死后端白写一份 |
...
| 编号 | 内容 | 归属 | 状态 |
| --- | --- | --- | --- |
| F0 | LLM planner + 本地调用留痕 + 坏任务实验报告 | §2 | 🟡 第一轮完成；需 provider 稳定后复测 Gate 样本 |
...
| T1 | Ink 终端 UI 只读三面板（状态/chat/actions） | T 独立线 | ⚪ 可随时插 |
| T2/T3 | Ink 交互（审批依赖 F1.3）与补齐 | T 独立线 | ⏸ T2 排 F1 后 |
| C4 | 前端 zh/EN i18n | PLAYBOOK 同名条目 | ⚪ |
| E3 | 暗色模式 + 首次引导 | PLAYBOOK 同名条目 | ⚪ |
| E0-e2e | Hardware/注册/DangerZone/ConfigPanel 的 Playwright 用例 | PLAYBOOK 同名条目 | ⚪ |
| B4-vec | 真实向量 RAG（sqlite-vec + embedding） | 延后项；原设计见 git 历史 optimization-spec §4 | ⏸ |
| 基建 | CI（pytest 3.11/3.12 + 前端 build）；push 纪律；测试 env-scrub fixture | 流程欠账 | ⚪ |
