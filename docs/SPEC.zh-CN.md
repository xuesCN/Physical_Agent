...
| T4.1 | MOCE 对话式 transcript、动作记录与终端宽度修复 | `superpowers/specs/2026-07-19-tui-conversational-transcript-followup-design.md` | 🟡 2026-07-19 设计已批准、待实现：仅修改 Ink TUI；修复 finalized/streaming 悬挂缩进与 display-width 门禁，角色轨道切换为不同颜色的 `>` / `⏺`，从 canonical Action Board 追加本地会话内真实 terminal action，pending approval 只读提示继续使用既有 `/approve`、`/reject`。不改 API/SQLite/watch/driver/SafetyGate/SAFETY.md 或审批语义，不显示非权威完成时间，不解冻其他功能。 |
