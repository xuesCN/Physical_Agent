...
**验证（2026-07-20 T4.1 本地门禁）**：controller 在 `e0215cb` fresh gate 中得到 TUI typecheck exit 0、full `108/108`、build exit 0、activity/render/scenario 专项 `70/70`；Python safety `27/27`，full `463 passed` 且仅 1 条已知 Starlette `TestClient` deprecation warning；`git diff --check` 与禁止 TUI driver/watch/sqlite import 扫描均 clean。逐任务终审和整体验收均为 Critical=0、Important=0、Minor=0，整体结论 Ready to merge=Yes。远端 exact-head Push/PR CI 尚待实际 push 后观察，本地闭环不预写 run ID 或 head SHA。
