...
| 编号 | 内容 | 归属 | 状态 |
| --- | --- | --- | --- |
| **R0-R8** | 架构减法与兼容面退役 | `specs/001-architecture-simplification/` | 🟡 R0-R2.1、R3、R4 已完成。R3 完成 sidecar 接管、历史救援、migrator/full Workspace 删除与发布形态收口；R4 `da14064` 删除 CLI/API/ChatRuntime 的 `auto_step` 兼容形状和 CLI 内嵌 watch composition，改用直接的 request/proposal 执行边界测试，正式 `watch` 与 `api --watch` 保留。下一阶段 R5 structured Chat Turn 双轨，不提前删除 `action-draft` fence |
