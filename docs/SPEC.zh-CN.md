...
| 编号 | 内容 | 归属 | 状态 |
| --- | --- | --- | --- |
| **R0-R8** | 架构减法与兼容面退役 | `specs/001-architecture-simplification/` | 🟡 R0-R2.1、R3 已完成。R3-A～E 完成 sidecar 接管、真实历史救援、当前 migrator 与 full Workspace 删除；R3-F `9698b3e` 清理当前文档并修复增量 wheel 陈旧模块泄漏，wheel 明确拒绝已退役 GUI/migrator/full Workspace 文件。真实 `9072b4e` 救援、全量 Python `404 passed`、Safety、frontend、TUI 与 clean-wheel 均通过。下一阶段 R4 退役 `auto_step` |
