...
| F2.5 | State Overview 产品化收口 | §2/F2 后续收口 | ✅ 2026-07-08 完成：新增 `frontend/src/viewmodels/` formatter 层；Overview 首屏用 AntD Card/Statistic/Table/Descriptions/Tag/List 展示 system/robots/capabilities/environment/world objects；`RawDebug` 仅折叠展示 unknown/raw/backend private 字段，`react18-json-view` 继续只作 fallback；Playwright mock 覆盖产品化组件与默认折叠。同日 follow-up：非 RawDebug 的 params/schema/raw 展示统一为 Overview capability 风格的轻量摘要，JSON tree 只剩 RawDebug |
