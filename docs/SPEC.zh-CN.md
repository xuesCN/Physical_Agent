...
| 编号 | 内容 | 归属 | 状态 |
| --- | --- | --- | --- |
| **R0-R8** | 架构减法与兼容面退役 | `specs/001-architecture-simplification/` | 🟡 R0-R2.1 已完成。R3-A `e7f878b` 锁定 sidecar behavior；R3-B `a64be59` 完成 adapter cutover；R3-C `d1713a0` 固化真实历史救援；R3-D `6309a1b` 删除当前 CLI migrator、reader、config bypass、migration-only audit/SQLite helpers 与对应测试。删除后 `9072b4e` 独立 worktree 救援仍全绿，legacy 显式/隐式入口继续 fail closed，全量 Python `415 passed`。下一阶段 R3-E 退役 full Workspace helper/protocol |
