...
| T4.2 | finalized assistant 块级 Markdown 与嵌套列表呈现 | `docs/superpowers/specs/2026-07-20-tui-block-markdown-renderer-design.md` | 🟡 2026-07-20 final whole-branch repair 本地完成，待新 exact-head CI：`f1ed6c7` 以 document-wide reference label 集合修复 matching shortcut/image shortcut 隐藏源标记，并复用 scanner fence consumption 忽略代码块内伪 definition；最终 re-review Critical=0、Important=0、Minor=1，唯一 Minor 为已接受的非阻塞 Windows Terminal smoke omission。`f1ed6c7` exact-commit 报告：TUI parser+renderer `60/60`、full `134/134`、typecheck/build exit 0，Python full `463 passed, 1 warning`；closure controller fresh docs+safety `24/24`、safety `27/27`。旧 `d72e187`/evidence-only `a89be06` workflows 在当时树上有效，但已被本修复 supersede，不是新 final head 的 closure 证据。只修改 TUI presentation/docs，不改变 API、watch、driver、SQLite、SafetyGate、SAFETY.md、审批或冻结状态。 |
