...
## 2. 当前目标（R0-R8.1 与 T4 已收口；Phase F 其余功能扩张冻结）
...
| **T4** | MOCE TUI 品牌与终端视觉层级 | ✅ 2026-07-17 完成本地验收；只改 Ink TUI 品牌、布局、finalized text 呈现与宽度兼容，不改 API、watch、driver、SafetyGate、命令或状态语义；设计见 `docs/superpowers/specs/2026-07-17-tui-moce-visual-refresh-design.md`，实现与门禁证据见 §4 与 `REFACTORING.zh-CN.md` |
...
| T4 | MOCE TUI 品牌与终端视觉刷新 | `PLAYBOOK` T4 + `superpowers/specs/2026-07-17-tui-moce-visual-refresh-design.md` | ✅ 2026-07-17 完成：响应式 MOCE Banner `7a968d9`；单一 Static feed 与角色轨道 `69615dd`；finalized assistant 轻量格式化及 fail-closed review fixes `ed7cc4a`、`4fb0f57`、`e4ea0d4`、`c9981f0`、`a3d1d22`、`7e391b3`、`1d758f5`；终端层级 `83d9efe`；宽度/物理 stdout 验收与语义同步 `88f7b5d`、`d25c80e`、`e9f886f`，测试输入同步 `a8deeac`，non-CI render test child `647ce53`；文档闭环 `8e30a42`。最终本地门禁：TUI typecheck、render `30 passed`、scenario `16 passed`、API-only safety `1 passed`、full `84 passed`、build 全绿；两个物理 stdout 用例额外连续复跑 3 轮全绿；Python safety `2 passed`，full `463 passed, 1 warning`；`git diff --check` clean；独立终审 Critical=0、Important=0、Minor=0。首次 exact-head `038c1c6` 的 Push run `29629559330` 与 PR run `29629560483` 均在 Enter 前失败；`a8deeac` 改为等待测试 stdin 被真实消费。第二次 exact-head `7d380f9` 的 Push run `29630023334` 与 PR run `29630024721` 均推进到 Enter 后才失败，确认 Ink v5 在 `isInCi && debug:false` 时只即时写 Static、动态 frame 延迟到 unmount；两轮 Push 中已执行的 safety、frontend+wheel 成功且条件型 Python full/Playwright skipped，两轮 PR 中 Python full/safety、frontend+wheel 与 Playwright 成功。`647ce53` 只把 spawned Node test child 的 `CI` 规范化为 `false`，让物理 stdout 契约在本地与 hosted runner 都走 Ink 非 CI 动态渲染分支（测试 harness 仍模拟 `isTTY=true`）；typecheck/build 与产品 runtime 仍处于真实 CI 环境。`CI=true npm.cmd test` 已完成 82/84 RED → 84/84 GREEN，独立复核无阻断（Critical=0、Important=0；Minor=1：未来 CI 专属行为需另开保留 `CI=true` 的子进程）。最终远端 closure：implementation/docs head `9c50067` 的 Push run `29630361521`、PR run `29630363135` 均 completed success，headSha 均为 `9c50067964e5ea184803e83526121388078862f9`；PR 中 Python 3.12 full/safety、Ink TUI、frontend+wheel 与真实 Chromium Playwright 全部成功。phase-B evidence-only 提交只记录既有成功证据，不预写自身 run。实现仍是纯 HTTP API/SSE 客户端，未改变 API/watch/driver/SafetyGate/Action Board/审批语义，Phase F 其余功能继续冻结。 |
| T4.1 | MOCE 对话式 transcript、动作记录与终端宽度修复 | `superpowers/specs/2026-07-19-tui-conversational-transcript-followup-design.md` | ✅ 2026-07-20 完成：`f81b134` 收紧宽度并共享 `MessageRow`，`216a0b9` 增加 canonical terminal action activity，`8139d24` 固化自有不可变 action snapshot，`c2f8c7c` 隔离单项 clone failure，`e0215cb` 完成 App 集成与只读审批提示。`e0215cb` 上 controller fresh gate：TUI typecheck exit 0、full `108/108`、build exit 0、activity/render/scenario 专项 `70/70`；Python safety `27/27`，full `463 passed` 且仅 1 条已知 Starlette `TestClient` deprecation warning；`git diff --check` 与 TUI 禁止 driver/watch/sqlite import 扫描均 clean。逐任务终审及整体验收均为 Critical=0、Important=0、Minor=0，整体结论 Ready to merge=Yes。implementation/docs exact head `357a4898fc19704179e15c1614f49e49f794263f` 的 Push run `29695566978`、draft-PR run `29695568726` 均于 attempt 1 completed success：Push 的 Ink TUI、frontend+wheel、Python safety 成功且 PR-only jobs skipped；PR 的 Ink TUI、frontend+wheel、Python 3.12 full、Python safety 与真实 Chromium Playwright 成功，generic matrix job skipped。FastAPI、SQLite、action/feedback wire、watch、driver、SafetyGate、SAFETY.md、parser/审批、时间戳与冻结状态均未改变；既有 `/approve`、`/reject` 语义不变，未新增 y/n/a 或 session grant。 |
| T4.2 | finalized assistant 块级 Markdown 与嵌套列表呈现 | `docs/superpowers/specs/2026-07-20-tui-block-markdown-renderer-design.md` | ✅ 2026-07-20 final whole-branch repair 完成：`f1ed6c7` 以 document-wide reference label 集合修复 matching shortcut/image shortcut 隐藏源标记，并复用 scanner fence consumption 忽略代码块内伪 definition；最终 re-review Critical=0、Important=0、Minor=1，唯一 Minor 为已接受的非阻塞 Windows Terminal smoke omission。`f1ed6c7` exact-commit 报告：TUI parser+renderer `60/60`、full `134/134`、typecheck/build exit 0，Python full `463 passed, 1 warning`；closure controller fresh docs+safety `24/24`、safety `27/27`。implementation/docs exact head `6631f35b22eba6c6e57394c07fec3a26254aa686` 的 Push run `29728361574` 与 draft-PR run `29728365131` 均 attempt 1 completed success。旧 `d72e187`/evidence-only `a89be06` workflows 在当时树上有效，但已被本修复 supersede，不是 final repair closure 证据。只修改 TUI presentation/docs，不改变 API、watch、driver、SQLite、SafetyGate、SAFETY.md、审批或冻结状态。 |
| C6 | Dashboard 信息架构重排 | `design/ui-redesign-spec.zh-CN.md` A.1-A.5 / PLAYBOOK 同名条目 | ✅ 2026-07-21 完成：导航由 9 项收敛为 `chat/overview/actions/state/hardware/memory/events/settings` 8 项，确认新页覆盖后退役 `world/safety/robots`；`state` 独占 world/feedback/safety/capabilities 四页签，`actions` 只渲染 feedback；ProposalPanel 按 page key 记忆，Actions 默认展开、其余默认收起且收起时不挂载；ActionBoard 落地空/普通 pending/待审批三态，遍历 `pending.metadata.approval.required` 计数，待审批强制展开、无折叠入口并保留 warning 与批准/拒绝按钮，窄屏时审批信号优先于提案抽屉。验收：frontend `tsc -b && vite build` 通过（3309 modules）；真实 Chromium `33 passed`（原 28 + C6 5）；Python full `469 passed, 1 warning`；Safety smoke `32 passed`；TUI typecheck/build 通过、full `138 passed`；clean-wheel smoke 通过；最终代码复审 Critical/Important/Minor 均为 0。2026-07-21 布局收口：桌面对话页改为占满视口可用高度、消息区内部滚动，收起态提案栏的箭头/图标/纵向文字居中；隔离 `.tmp/e2e` 后端的真实 Chromium `34/34`、frontend build 与 Python `469 passed` 通过。只改 Dashboard/前端测试与文档，不改 Python、API/SSE、ChatPlan、TUI 协议、watch/driver/SafetyGate/SAFETY.md |
| C6.1 | Dashboard 纯前端 URL 页面状态 | PLAYBOOK 同名条目 | ✅ 2026-07-21 完成：以根路径查询参数 `?page=<page-key>` 作为页面选择的 URL 表示，支持直达、刷新保留和浏览器前进/后退；缺省 `/` 仍显示 overview 且不改写 URL，非法及已退役 key 通过 `replaceState` 归一到 overview；导航保留其他 query 参数，同页点击不增加 history。Sidebar 与 action 内跳转复用同一前端 navigation hook，Dashboard/SSE 保持常驻。验收：frontend `tsc -b && vite build` 通过（3310 modules）；隔离 `.tmp/e2e` 后端的真实 Chromium `37/37`；Python full `469 passed, 1 warning`；Safety smoke `32 passed, 1 warning`；TUI typecheck/build 与 full `138/138`；clean-wheel smoke 通过；in-app Browser 复核直达、刷新、Back/Forward、非法参数归一及 console 0 warning/error。只改 `frontend/`、前端测试、发布静态产物与文档，不新增路由/状态管理依赖，不改 FastAPI、API/SSE、SQLite、watch、driver、SafetyGate 或 SAFETY.md。 |
