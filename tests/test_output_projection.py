...
def test_projection_uses_final_claim_owner_gate_after_late_prior_owner_event():
    action = _compiled().actions[0]
    output = materialize_agent_output(
        _compiled(),
        actions={
        ...
        feedback={
            "history": [
                ...
                ),
                {
                    ...
                    "executor_id": "watch-successor",
                    "status": "completed",
                    "message": "completed by the successor",
                    "result": {},
                    "artifacts": [],
                },
                _gate_event(
    ...
    assert tasks["safety_gate"].status == "passed"
    assert tasks["safety_gate"].details["last_decision"]["executor_id"] == (
        "watch-successor"
    )


def test_projection_fails_closed_when_completed_final_claim_owner_is_missing():
    action = _compiled().actions[0]
    output = materialize_agent_output(
        _compiled(),
        actions={
...
def test_projection_rejects_latest_invalid_event_from_final_claim_owner():
    action = _compiled().actions[0]
    invalid_current_owner = _gate_event(
        status="passed",
        decision="allow",
        executor_id="watch-successor",
    )
    invalid_current_owner["actor"] = "agent"
    output = materialize_agent_output(
        _compiled(),
        actions={
            "pending": [],
        ...
        feedback={
            "history": [
                ...
                ),
                {
                    ...
                    "executor_id": "watch-owner",
                    "status": "failed",
                    "message": "dependency became impossible",
                    "result": {},
                    "artifacts": [],
                },
            ]
