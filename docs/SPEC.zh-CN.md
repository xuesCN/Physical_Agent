...
| 编号 | 内容 | 归属 | 状态 |
| --- | --- | --- | --- |
| **R0-R8** | 架构减法与兼容面退役 | `specs/001-architecture-simplification/` | 🟡 R0 完成 `e8750ca`；R1.5 cutover 经 draft PR #1 / `2d5e909` 的 Python full、Safety、TUI、frontend、clean-wheel 和真实 Chromium 25/25 验证。R2 已删除 `physical_agent/gui/`、旧 server/static/tests/package-data 与安全 allowlist 例外，正式 GUI 只剩 FastAPI + packaged React；最终回归后进入 R3 sidecar/migration 退役 |
